...
import { applyBuyPrice, bestSellPrice, type MarketStocks } from './market'
...
export function sellPrice(itemId: string, reputation = 0): number {
  return bestSellPrice(STOCKS as MarketStocks, itemId, reputation)
}

/** V1.4 买价：商店标价经风评系数折算（负风评买贵） */
export function buyPrice(price: number, reputation = 0): number {
  return applyBuyPrice(price, reputation)
}