...
| GFX-3 | 角色 4 方向行走动画帧 | M1 | ⬜ | G4 截图比对 | 待 rich-graphics 分支 |
| GFX-4 | 动态环境（水面波动/树木摇曳/花草点缀） | M1 | ⬜ | G4 | 同上 |
| GFX-5 | 场景装饰物（房屋/桥/篱笆/门匾）与色调氛围 | M1 | 🚧 world-maps(mapTiles.ts) | G4 | world-maps |
| GFX-6 | 区域进入横幅 + 平滑过场淡入淡出 | M1 | 🚧 world-maps | G4 | world-maps |
...
| ID | 模块项 | 里程碑 | 状态 | 验证 | 证据/分支 |
|---|---|---|---|---|---|
| MAP-1 | 地图 JSON DSL + schema + 校验（行走性/portal/引用） | M1 | 🚧 world-maps | G1 | world-maps |
| MAP-2 | 七玄门山村（新手村）手作地图 | M1 | 🚧 world-maps | G1/G4 | world-maps |
| MAP-3 | 小径山道（连接图） | M1 | 🚧 world-maps | G1/G4 | world-maps |
| MAP-4 | 密林妖谷（妖兽区） | M1 | 🚧 world-maps | G1/G4 | world-maps |
| MAP-5 | portal 场景切换 + 出生点定位 + 存档恢复地图 | M1 | 🚧 world-maps | G4 | world-maps |
...
| DIA-4 | 对话内容量产（全部可交互 NPC 有对话） | M3 | 🚧 内容部分就绪（main 31e3db3），绑定进行中 | G1/G4 | world-maps 绑定 4 名 |
...
| CBT-3 | 功法技能实装（品阶威力档位、回复/增益类） | M1 | 🚧 combat-depth | G3 | combat-depth |
| CBT-4 | 战斗中用物品（回气散等消耗品） | M1 | 🚧 combat-depth | G3/G4 | combat-depth |
| CBT-5 | 妖兽多样性 + Boss（怒气机制）+ 掉落表 | M1 | 🚧 combat-depth | G1/G3 | combat-depth |
| CBT-6 | 经验/升级成长曲线 + 境界显示联动 HUD | M2 | 🚧 combat-depth | G3/G4 | combat-depth |
| CBT-7 | 属性/异常状态扩展（中毒 DOT 等） | M2 | 🚧 combat-depth（毒蛛） | G3 | combat-depth |
...
| QST-2 | 任务运行时（接取/进度/完成/奖励状态机） | M1 | ⬜ → 🚧 quest-engine 分支 | G3/G4 | 本次启动 |
| QST-3 | 触发器接线（对话/击杀/采集/到达 → 进度事件） | M1 | ⬜ → 🚧 quest-engine | G4 | 本次启动 |
| QST-4 | 任务日志 UI（进行中/可接取/已完成）+ HUD 追踪提示 | M1 | ⬜ → 🚧 quest-engine | G4 | 本次启动 |
| QST-5 | 主线第一章任务链端到端跑通 | M1 | ⬜（依赖 QST-2~4） | G4 | — |
...
| INV-2 | 真实背包接入 content/ + 存档（拾取/掉落入包） | M2 | 🚧 combat-depth（player.ts + item:acquired） | G3/G4 | combat-depth |
...
| DAT-3 | 妖兽图鉴数值平衡（对照 GDD 锚点） | M2 | 🚧 combat-depth 新增 3 只 | G1/G3 | combat-depth |
...
| QA-3 | 本地 E2E 脚本入库 `scripts/qa-local.mjs`（覆盖 G4 全清单） | M1 | ⬜ → 集成期落地 | G4 | 集成分支 |
| QA-4 | 线上 QA 自动化 + 版本去重 | M0+ | ✅ | G5 | qa-automation 工作树 |
| QA-5 | 性能预算（首屏 JS gzip ≤400KB，60fps 移动端） | M4 | 👀 当前 ~392KB 达标，持续观察 | G2/G5 | combat-depth 实测 |
...
| M1 可玩垂直切片 | 多地图世界+对话+战斗+任务链+存档 UI | 🚧 集成中（world-maps / combat-depth / quest-engine / rich-graphics） |
