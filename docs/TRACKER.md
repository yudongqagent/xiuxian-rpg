...
| CBT-5 | 妖兽多样性 + Boss（怒气机制）+ 掉落表 | M1 | ✅ | G1/G3 | 全 13 只妖兽 exp/loot 完整（含 special: poison/enrage），掉落表覆盖全品阶 |
...
| QA-7 | 数值节奏审计（经验曲线 vs 章节境界门槛模拟） | M4 | ✅ | 分析 | 主线-only 玩家 qm_04 前需 ~70 场刷怪（鼓励支线）；主线+支线经验 15100 ≫ L20 需 3990，无需调数值 |
...
| 2026-08-22 | hotfix-pt2：第二轮对抗性测试修复——妖兽呼吸改缩放脉冲（Y 轴追击解锁）、传送落点错位修复（山道东返回弹跳）、李飞宇对话补全、隐藏支线转正、Esc 关商店、逃跑真实位移+免战 2.2s | 已发布 |

| 2026-08-22 | audit-balance：全链经验节奏模拟——支线经验 7250 补足主线刷怪缺口，数值无需调整；QA-7 入库 | 已发布 |
...
| 2026-08-24 | feat-immersion：凡人流核心三件套+一致性修正——①突破仪式：大境界门槛（13/17/21/25 级）修为封顶，需丹药（筑基丹/黄龙丹/洗髓丹）+灵气圆满+机缘加成（洞府+15%），概率突破、失败耗丹重伤不堕境界；境界扩展至化神（qm_07~10 此前因 realmLabel 封顶筑基圆满而永久锁死）；②打坐产修为（每两跳一修为×灵气密度）；③掌天瓶药圃：瓶液真实时间 30 分钟一滴（上限5）、播种灵草 10 分钟成熟、绿液一滴催熟全部、采收一株化三，背包新增药圃 Tab，起始附神秘小瓶；④一致性：主角更名韩立、厉飞雨野狼帮之乱诀别对话、南宫婉改掩月宗、黑衣试炼者改各派弟子（血色禁地道德灰度）、金光上人改邪道散修。combat-check +10 项、qa-local 18/18 | 待发布 |
...
| 2026-08-30 | V1.3 日程化 NPC + 关系图雏形：①NpcScheduleSchema（时辰坐标表/watchRadius + npcSpotAt 落点查表）；②新 NPC 张二（杂役院盯梢）/李三（七玄门坊市）+ 莫大父药园↔诊堂日程；③relations.ts（affinity/grudge 钳制，GRUDGE_PER_THEFT=1）；④WorldScene 日程落位 + time:state 位移 tween + tryGather 目击判定→记恨+1 + 快照持久化 relations；⑤debug 钩子 npcs()/relations()/time.get·advance·set；⑥validate checkSchedules · sandbox-sim 33 项 · qa-local 场景 8.91 → 26/26。G1–G4 全绿 | 已发布 |
| 2026-08-30 | V1.4 事件风暴首例「杂役院失窃」：①WorldEventSchema（trigger: absentDays/grudgeOf/grudgeAt + consequences: lingshi/reputation + nominee/once/toast）；②worldEvents.ts 纯函数（旷工计数·条件求值·后果结算）+ 坊市风评可订阅存储 + worldEventLoader（Vite glob，tsx 下兜底空表）；③WorldScene time:state 触发 + tryGather 上工记录 + 快照 events/labor/reputation 入档恢复 + debug 钩子 world()/relations.bump；④风评→坊市物价（GDD §3：buy/sellPrice ± 风评系数钳 0.7~1.3，ShopPanel 徽标）；⑤validate checkEvents · sandbox-sim 51 项 · qa-local 场景 8.92 事件触发+后果+入档不重触发 → 29/29。G1–G4 全绿 | 已发布 |
| 2026-08-30 | V1.5 寿元推进：①世界历驱动 lifespan.ts（age=22+floor((day-1)/60)，境界→寿元表 炼气120/筑基200/结丹400/元婴800/化神1500）②突破失败且余寿<半寿 → 大境界硬锁「寿元剩余 37 年无望筑基！道基已定，此生止步。」+ 此世无缘/天命已绝态 ③寿元耗尽=终局「寿尽而终」（世界时钟冻结，结局入档可恢复）④闭关参悟一键跳过等待（闭关一载=+60日=耗寿1载，按灵气密度结算修为，道具丹药仍可加速省寿元）⑤HUD 寿余倒计时 ⑥调试钩子 rng.force/realm.set/world().aging；sandbox-sim 70 项 · qa-local 场景 8.93/8.94/8.95 → 34/34。G1–G4 全绿 | 已发布 |
| 2026-08-30 | V1 出口验收（§5.3）：①涌现行为矩阵 sandbox-sim §17（啃老/挖药/抢人/炼丹/内卷 × 30 日，复用 gather/combat/alchemy/worldEvents 纯函数，结局两两不同；内卷先偷后躺触发「杂役院失窃」）②坊市价格纯函数抽取 src/systems/market.ts（Node/browser 共用口径）③preview 自动试玩旁路 scripts/playtest-observation.mjs 产出具体观察：张二盯梢因果随时辰表切换（辰时偷摘记恨+1，午时无人知，记录入 PROGRESS §V1 出口验收）+ 炼丹入不敷出平衡警示（124<136）；门禁 validate ✓ build ✓ combat ✓ quest 52/52 sandbox 75/75 qa-local 34/34 → V1 里程碑达成 | 已发布 |
| 2026-08-30 | V2.1 恩仇系统（§6.1）：恩仇类型 type（mentor/rival/debt/spouse/enemy）阈值推导·仇重于恩；送礼 G 键（首礼+8/七日内只+1 防刷）+ 偷摘写回恶化（记恨+1 好感-2）；事件风暴通用化（trigger 族=旷工/记恨/好感可组合，consequences=赠灵石/清记恨/还人情），新增「张二报恩」（好感≥40赠12还情债）「张二寻仇」（记恨>6扣15风评-5清偿）；docs/BENCHMARK.md 建立开放世界休闲标杆对照（社交核心循环金线）；门禁 validate ✓ build ✓ sandbox 91/91 combat ✓ quest 52/52 qa-local 38/38（8.96 送礼报恩 / 8.97 寻仇）试玩观察二·善缘vs药园经济成立 | 已发布 |
| 2026-08-30 | V2.2 NPC 世界级生命周期（§6.2）：打探[H]——自定义 probe 优先、缺省日程自动折叠白话（连续时辰合并/多段分段），配 cultivate 附现实力+寿元大限年；NPC 亦修真——world 历每 growth 年升 1 层至封顶（茶翁凡躯60/青纹炼气80/万小山90/陈巧倩筑基200/李化元结丹400）；坐化 once 入档重载不重播，好感≥30 有悼文，坐化者不可再打探/收礼；剧情锚免疫死亡（validate 强制断言张二/李三/墨大夫不可配 cultivate）；门禁 validate ✓ build ✓ sandbox 99/99 combat ✓ quest 52/52 qa-local 48/48（8.98 打探/洞察/演进/坐化 10 断言）试玩观察三·NPC 也是会老的修真者成立 | 已发布 |
| 2026-08-31 | V2.3 危险度带（§6.3）：删章节锁（全图可达，5 处 lockQuest/lockHint 已移除）；RegionSchema danger { intel, levelMin }——6 区域投放（彩霞4/黄枫9/太南12/血色13/战场15/洞府18），首次踏入危险区弹坊间传闻 toast once（danger kind 红边+音效）+ seenWarnings 入档；战败软拦——危险区战败先落盘再 restart 七玄门山村(18,22)（气血折半持久，寿元走世界历不受影响）；安全区战败仅原地传送；battle.lose debug 钩子；WorldMapPanel 重写（危险区红框标注建议层数）；validate 新增传送点无章节锁残留断言；门禁 validate ✓ build ✓ sandbox 99/99 combat ✓ quest 52/52 qa-local 51/51（8.5 门户全开+传闻 once / 8.99 战败抬回+气血折半）试玩观察四·全图可达金线/信息差软拦/战败抬回山村成立 | 已发布 |
